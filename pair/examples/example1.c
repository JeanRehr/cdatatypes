...
void insert_grade(struct arraylist_grades_list *grades_list, char *name, int grade, struct Allocator *alloc) {
    struct pair_grades *tmp = grades_list_emplace_back(grades_list);
    // Test tmp here
...
int main(void) {
    ...
    struct pair_grades *tmp = grades_list_emplace_back(&student_grades);
    tmp->first = heap_alloc_from_str_lit("Alice", &alloc);
    tmp->second = 86;

    tmp = grades_list_emplace_back(&student_grades);
    tmp->first = heap_alloc_from_str_lit("Bob", &alloc);
    tmp->second = 98;

    // The constructor generated by pair header can also be used to assign directly:
    *grades_list_emplace_back(&student_grades) = grades_init(heap_alloc_from_str_lit("Ian", &alloc), 10);
    *grades_list_emplace_back(&student_grades) = grades_init(heap_alloc_from_str_lit("Ron", &alloc), 43);
    *grades_list_emplace_back(&student_grades) = grades_init(heap_alloc_from_str_lit("Telos", &alloc), 69);
    ...
    // *grades_list_emplace_back(&student_grades) = grades_init("test", 69); ///< error just because there is a destructor for char pointers
