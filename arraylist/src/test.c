...
static void test_arraylist_find(void) {
    ...
    struct test *found = arraylist_test_find(&arrlisttest, test_pred_find_name, (char*)target_name_found);
    assert(found == arraylist_test_at(&arrlisttest, 1));

    const char *target_name_not_found = "add3";
    struct test *not_found = arraylist_test_find(&arrlisttest, test_pred_find_name, (char*)target_name_not_found);
...
ARRAYLIST(struct test_simple*, test_simple_ptr)
...
static void test_simple_arraylist_ptr_push_back(void) {
    ...
    *add3 = test_simple_ctor_by_val(12, 0.7, 3);
    ...
    assert(arrlisttestptr.size == 1);
    arraylist_test_simple_ptr_push_back(&arrlisttestptr, add2);
    assert(arrlisttestptr.size == 2);
    assert(arrlisttestptr.capacity == 2);
    arraylist_test_simple_ptr_push_back(&arrlisttestptr, add3);
    assert(arrlisttestptr.size == 3);
    assert(arrlisttestptr.capacity == 4);
    arraylist_test_simple_ptr_push_back(&arrlisttestptr, add4);
    assert(arrlisttestptr.size == 4);
    assert(arrlisttestptr.capacity == 4);
    arraylist_test_simple_ptr_push_back(&arrlisttestptr, add5);
    assert(arrlisttestptr.size == 5);
    assert(arrlisttestptr.capacity == 8);

    arraylist_test_simple_ptr_reserve(&arrlisttestptr, 11);
    assert(arrlisttestptr.capacity == 11);
...
static void test_simple_arraylist_ptr_emplace_back_slot(void) {
    ...
    *slot_add1 = add1;
    ...
    struct test_simple **add1_same = arraylist_test_simple_ptr_at(&arrlisttestptr, 0);
...
int main(void) {
    ...
    test_simple_arraylist_emplace_back_slot();

    test_simple_arraylist_ptr_push_back();
    test_simple_arraylist_ptr_emplace_back_slot();
