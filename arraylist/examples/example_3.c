...
int main(void) {
    ...
    *add1 = non_pod_init(&gpa, 228, 421, 244);
    ...
    if (np_push_back(&vec_np, add1) != ARRAYLIST_OK) {
    ...
    for (size_t i = 0; i < np_size(&vec_np); i++) {
    ...
    for (struct non_pod **it = np_begin(&vec_np); it < np_end(&vec_np); ++it) {
        printf("value %d\n", *(*it)->_number);
    }
    ...
    *np_emplace_back_slot(&other) = non_pod_init_alloc(&gpa, 11992, -1000, 101010);
    ...
    struct non_pod **not_foundref = np_find(&vec_np, non_pod_find, (void *)10);

    // If value is not found, it will return the end of the arraylist, dereferencing it is UB
    if (not_foundref == np_end(&vec_np)) {
        printf("not found\n");
    }

    struct non_pod **foundref = np_find(&vec_np, non_pod_find, (void *)1000);
    if (foundref != np_end(&vec_np)) {
        printf("found, value is: %d\n", *(*foundref)->_number);
    }

    // Contains returns false or true, and an optional index where it was found
    size_t index = 0;
    bool found = np_contains(&vec_np, non_pod_find, (void *)2000, &index);
