...
size_t names_read_lines(struct arraylist_names_arr *names_arr, FILE *stream, struct Allocator *alloc) {
    size_t num_lines = 0;
    char *line;
    ...
    while ((line = read_line(stream, alloc)) != NULL) {
        ...
        *arraylist_names_arr_emplace_back(names_arr) = line;
        num_lines++;
    }
    printf("\n");
    return num_lines;
...
int main(void) {
    ...
    // *arraylist_names_arr_emplace_back(&names_arr) = "Testing";
    ...
    *arraylist_names_arr_emplace_back(&names_arr) = str;

    // Using a constructor
    *arraylist_names_arr_emplace_back(&names_arr) = heap_alloc_from_str_lit("Full Name", &gpa);
